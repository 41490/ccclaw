...
| 来源 | SHA256 | 版本 |
|------|--------|------|
| `.release/` 上传资产 | `5523b4ce...` | `26.03.16.1742` |
| `/ops/logs/ccclaw/26.03.16.1742/` 本地缓存 | `5523b4ce...` | `26.03.16.1742` |
| GitHub release 下载资产 | `5523b4ce...` | `26.03.16.1742` |
...
| 检查项 | 预期 | 实际 | 结论 |
|--------|------|------|------|
| `scheduler status` | `request=systemd effective=systemd` | `request=systemd effective=systemd` | ✓ |
| `list-unit-files 'ccclaw*'` | 10 个 unit | 10 个 unit | ✓ |
| `list-timers --all 'ccclaw*'` | 5 个 timer | 5 个 timer | ✓ |
| `doctor` | 18 项通过 | 18 项通过 | ✓ |

## 结论

### 本次发布与升级链路结论

- release 版本、缓存资产、安装后二进制版本三者严格一致
- 升级后现有配置 `config.toml` 未被错误迁移或重写
- `home_repo` 用户记忆（`/opt/data/9527`）HEAD 与脏文件集合升级前后不变
- `scheduler status` 仍为 `request=systemd effective=systemd`
- `doctor` 18 项全部通过
- systemd unit 与 timer 数量不变
...
本轮发布/升级链路本身正常，但运行态仍可见历史 Issue 被 Claude 新 `system/hook_started` 类事件误判为 `FAILED/DEAD` 的残留迹象。该问题已有独立议题跟踪：

- Issue #75 `forensics: Claude 2.1.76 system stream 事件导致成功任务被误判 FAILED/DEAD`

该问题不阻塞本次 release 交付，但建议继续人工复核一次 gh-issue 最小链路，确认部署面正常、异常仅限于既有 stream 兼容问题。
